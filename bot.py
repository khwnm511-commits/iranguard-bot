# -*- coding: utf-8 -*-
# 🇮🇷 IRAN COLLAPSE — شبیه‌ساز مدیریت، فروپاشی و بازسازی ایران
...
def tg(method, **kw):
    ...
    for _ in range(4):
        try:
            ...
            r = _SESS.post(f"{API}{TOKEN}/{method}", json=kw, timeout=t)
            j = r.json()
            if not j.get("ok"):
            ...
            print("[tg]", method, repr(e)); time.sleep(2)
    return {"ok": False}
...
def join_text():
    return "\n".join(["🔒 <b>عضویت در کانال اجباری است</b>", "━" * 20,
                      "اول عضو کانال شو - بعد برگرد بازی:",
                      "📰 اخبار و آپدیت‌ها + آموزش کامل ۹ بخشی", "━" * 20,
                      "بعد از عضویت، «🔄 بررسی عضویت» را بزن."])
...
CMDMAP = {"کشور": "country", "منو": "menu", "شروع": "menu", "استارت": "menu", "وضعیت": "menu", "بازی": "menu", "خانه": "menu",
...
def on_msg(m):
    ...
    with _lock:
        ...
        if is_start:
            ...
            migrate(p); mark_dirty()
            if new:
                cap = "\n".join(["🇮🇷 <b>فرمانده خوش آمدی!</b>",
                                 "━" * 20,
                                 "ایران در آستانه فروپاشی است؛ خزانه $2,000، تنش جهانی بالا.",
                                 "تو مدیر ارشد کشور: اقتصاد، شهرها، منابع، دیپلماسی، دفاع و بحران.",
                                 "هیچ شکستی دائمی نیست - بازسازی همیشه ممکن است.",
                                 "━" * 20,
                                 "🏘 هر گروه تلگرام = یک دنیای مستقل - «🌍 دنیاهای من» در منو",
                                 "قدم اول: مسیر سیاسی ایران را انتخاب کن 👇"])
                sent = False
                if COVER:
...
def main():
    ...
    print("▶ Bot: @" + (me["result"].get("username") or "") + " | IRAN-COLLAPSE v6.40")
    def _bye(sg, fr):
        try:
